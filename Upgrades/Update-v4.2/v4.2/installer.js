	...
	(function(){
		 ...
		 success:  function(result) {
			 ...
			 if (result.success) {
				 ...
				 $('#containerCreateUser, #headerCreateUser').show();
...
var element = $(this);
element.attr({'disabled' : 'true'});
element.html('<i class="spinner-border spinner-border-sm align-middle mr-1"></i> Creating...');
	 ...
	 success:  function(result) {
		 ...
		 if (result.success) {
			 ...
			 if (result.errors) {
				 ...
				 $('#showErrors2').html(error);
				 $('#errors2').show();
				 element.removeAttr('disabled');
				 element.html('Create User');
			 }
		 }
